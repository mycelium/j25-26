...
// Сериализатор: преобразует Java-объект в JSON-строку
class JsonSerializer {
    ...
    private void appendValue(StringBuilder sb, Object value) {
        ...
        if (value instanceof boolean[] arr)     { appendBoolArray(sb, arr); return; }
        ...
        if (value instanceof Collection<?> col) { appendCollection(sb, col); return; }
        if (value instanceof Map<?, ?> map)     { appendMap(sb, map); return; }
    ...
    private void appendBoolArray(StringBuilder sb, boolean[] arr) {
        sb.append('[');
        for (int i = 0; i < arr.length; i++) {
            if (i > 0) sb.append(',');
            sb.append(arr[i]);
    ...
    // Сериализуем произвольный объект через рефлексию
    private void appendObject(StringBuilder sb, Object obj) {
        ...
        for (Field field : getFields(obj.getClass())) {
            ...
            try {
                ...
                throw new JsonException("Не удалось прочитать поле: " + field.getName(), e);
            }
        }
        sb.append('}');
    }

    // Собираем все поля класса (включая унаследованные), пропуская static и transient
