...
public class HttpServer {
    ...
    private void handleConnection(SocketChannel channel) {
        try (channel) {
            ...
            try {
                ...
                response.setStatus(405, "Method Not Allowed").setBody("Method Not Allowed");
                channel.write(ByteBuffer.wrap(response.toBytes()));
                return;
            }

            HttpHandler handler = findHandler(request, method);
            if (handler != null) {
                try {
    ...
    private int indexOfHeaderEnd(byte[] data) {
    ...
    private HttpHandler findHandler(HttpRequest request, HttpMethod method) {
        for (Route route : routes) {
            if (route.method != method) continue;
            Map<String, String> params = matchPath(route.path, request.getPath());
            if (params != null) {
