...
Сервер построен на Java NIO (ServerSocketChannel). Selector в отдельном потоке принимает входящие соединения и передаёт каждое в пул воркеров, где происходит чтение байт, парсинг запроса и вызов зарегистрированного обработчика.
...
Invoke-WebRequest -Uri "http://localhost:8081/qs?name=Alice"
